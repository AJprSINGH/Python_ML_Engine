...
class JobRoleSkillRecommender:
    ...
    def prepare_collaborative_data(self):
        """Construct JobRole-Skill interaction matrix with improved handling"""
        if self.data is None:
            raise ValueError("No data for collaborative filtering. Please preprocess data first.")
        
        # Create interaction matrix with proficiency weighting
        interactions = self.data.groupby(['Job Role', 'Skill Title']).size().reset_index(name='count')
        
        # Add proficiency-based weighting if available
        if 'Proficiency Level' in self.data.columns:
            proficiency_map = {'beginner': 1, 'intermediate': 2, 'advanced': 3, 'expert': 4}
            proficiency_weights = self.data.groupby(['Job Role', 'Skill Title'])['Proficiency Level'].first().reset_index()
            interactions = interactions.merge(
                proficiency_weights, 
                on=['Job Role', 'Skill Title'], 
                how='left'
            )
        ...
        # Debug: Check what's in the interactions vs embeddings
        print(f"Unique job roles in interactions: {len(interactions['Job Role'].unique())}")
        print(f"Unique job roles in embeddings: {len(self.job_role_to_idx)}")
        print(f"Unique skills in interactions: {len(interactions['Skill Title'].unique())}")
        print(f"Unique skills in embeddings: {len(self.skill_to_idx)}")
        
        # Map job roles and skills to indices - only map those that exist in embeddings
        ...
        # Remove any rows with missing mappings with better error reporting
        ...
        if len(interactions) < initial_count:
            dropped_count = initial_count - len(interactions)
            print(f"Warning: Dropped {dropped_count} interactions due to missing mappings")
            
            # Show samples of dropped interactions for debugging
            dropped = interactions[interactions['role_idx'].isna() | interactions['skill_idx'].isna()]
            if len(dropped) > 0:
                print("Sample dropped interactions:")
        ...
        # Create sparse interaction matrix with weighted counts
        n_roles = len(self.job_role_to_idx)
        n_skills = len(self.skill_to_idx)
        
        print(f"Creating interaction matrix with dimensions: {n_roles} roles x {n_skills} skills")
        print(f"Total interactions to include: {len(interactions)}")
        
        # Verify all indices are within bounds
        max_role_idx = interactions['role_idx'].max()
        max_skill_idx = interactions['skill_idx'].max()
        
        if max_role_idx >= n_roles:
            print(f"Error: Max role index {max_role_idx} exceeds matrix dimension {n_roles}")
            return False
            
        if max_skill_idx >= n_skills:
            print(f"Error: Max skill index {max_skill_idx} exceeds matrix dimension {n_skills}")
            return False
        
        self.interaction_matrix = csr_matrix(
            (interactions['weighted_count'], (interactions['role_idx'], interactions['skill_idx'])),
            shape=(n_roles, n_skills)
        )
        
        # Add matrix density calculation
        matrix_density = (self.interaction_matrix.nnz / (n_roles * n_skills)) * 100
        print(f"Collaborative filtering data prepared. Density: {matrix_density:.2f}%")
    ...
    def generate_content_recommendations(self, job_role, top_n=5, min_similarity=0.1):
        """Generate content-based recommendations with quality thresholds"""
        ...
        if job_role not in self.job_role_to_idx:
            # Try fuzzy matching for similar job roles
        ...
        if len(valid_indices) == 0:
            return [{"skill": "No sufficiently similar skills found", "score": 0.0}]
        
        # Get top N skills above threshold
        valid_similarities = similarities[valid_indices]
        top_indices = valid_indices[np.argsort(valid_similarities)[::-1][:top_n]]
